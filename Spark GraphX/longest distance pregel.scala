...
object run {
    ...
    def main() : Unit = {
        ...
        val pregelresult = initialGraph.pregel(Double.PositiveInfinity)(
            (id, attr, msg) => math.max(attr.toInt, msg.toInt), //vertex program to store largest distance
        triplet => { 
       ...
       (a,b) => math.max(a,b))//merge program, largest distrance
        val result= pregelresult.vertices.sortBy(- _._2).take(21).foreach(println)
